...
public class Copy_3_of_BenchmarkRandomAccess {
	...
	public static void main(String[] args) {
		...
		try {						
			...
			final int nbInts = 50;
			final int[] positions = getRandomIntegers(nbInts);
				...
				//************ Roaring part ****************
			{
              ...
              //RAM storage
				...
				System.out.println("nbBitmaps = "+i_rb);
				...
				//Average time to perform 50 random accesses in each bitmap				
				double RandomAccessTime = test(new Launcher() {
					@Override
                    public void launch() {
						for(int i=0; i<irbs.length; i++)
							for(int j=0; j<nbInts; j++){
								boolean b = irbs[i].contains(positions[j]);
								careof+= b ? 1 : 0;
							}
                    }
				});
				...
				System.out.println("Random access time = "+RandomAccessTime+" ms");
			...
			{	
				...
				double RandomAccessTime = test(new Launcher() {
					@Override
                    public void launch() {
						for(int i=0; i<icss.size(); i++)
							for(int j=0; j<nbInts; j++){
								int b = icss.get(i).get(positions[j]);
								careof+=b;
							}
                    }
				});
	...
	static int[] getRandomIntegers(int nbInts) {
		int[] integers = new int[nbInts];
		Random rdm  = new Random(); 
		for(int i=0; i<nbInts; i++){
			integers[i] = rdm.nextInt();
		}
		return integers;
	}
